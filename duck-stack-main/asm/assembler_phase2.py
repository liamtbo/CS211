...
def assemble(lines: list[str]) -> list[int]:
    ...
    for lnum in range(len(lines)):
        ...
        try: 
            ...
            if fields["kind"] == AsmSrcKind.FULL: # AsmSrcKind distinguishses which kind of assembly language instruction we have matched
                log.debug("Constructing instruction")
                fill_defaults(fields) # fill in the default values for optional fields of instrution
                instr = instruction_from_dict(fields)
                word = instr.encode()
                instructions.append(word)
            elif fields["kind"] == AsmSrcKind.DATA:
                word = value_parse(fields["value"]) # parse an int literaly that could look like 42 or like 0x2a
                instructions.append(word)
            else:
...
def main(sourcefile: io.IOBase, objfile: io.IOBase):
    ...
    object_code = assemble(lines) # converts liens to a list of Instruction objects
