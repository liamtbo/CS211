...
def load(file: io.IOBase, memory: Memory) -> None:
    ...
    for line in file:
        ...
        memory.put(addr, word) # doesn't trigger self.hook-> "Quack!: value" bc addr != 511, & hooks_right is empty
...
def main(objfile: io.IOBase, display=False, single_step=False):
    ...
    mem = MemoryMappedIO(512) # gives the class self.mem = 512 * [0]
    ...
    mem.map_address_in(510, duck_input) # Quack! give me int input
    mem.map_address_out(511, duck_output) # Quack {value}
    cpu = CPU(mem)
    if display:
        log.debug("Creating a cpu_display")
        cpu_display = view.MachineStateView(cpu, 1200, 800)
    log.debug(f"Loading object file {objfile}")
    load(objfile, mem) # stores each encoded Instr line from file in memory with index +1 repecitvely as address (key)
