...
found in many cell phones, the Rasberyy Pi, and
...
offset_field = BitField(0,9) # signed field

"""
Registers are numbered from 0 to 15, and have names
like r3, r15, etc. Two special regigsters have additional
...
"""
The following operation codes control botht he ALU and somem
other parts oof the CPU.
ADD, SUB, MUL, DIV, SHL, SHR are ALU-only operation
...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        word = 0
        word = reserved.insert(0, word)
        word = op_field.insert(OpCode(self.op).value, word)
        word = cond_field.insert(self.cond.value, word) # this may take a long time
...
# Interpret an integer (memory word) as an instructiton.
# This is the decode part of the fetch/decode/execute cycle of the CPU

def decode(word: int) -> Instruction:
    """Decode a memory work (32 bit int) into a new Instruction"""
